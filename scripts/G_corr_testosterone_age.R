...
### REGRESSION OF GENETIC EFFECT, RESIDUALIZED BY AGE, ON TESTOSTERONE LEVEL - OVERALL ###
source("config.R")
...
for (pheno in pheno_list$Code) {
  ...
  # call function to get lm results for each bin
  m_results <- bin_fun(df_m,10,'male')
  f_results <- bin_fun(df_f,10,'female')
  results <- rbind(m_results, f_results)

  # correlation between genetic effect and testosterone
  corrs_result <- rbind(corrs_result, get_corr(results))
}

# save correlation file
setwd(GWAS_DIR)
write.table(corrs_result, file="pgs_testosterone_corr_age.txt", sep="\t", row.names=FALSE)
...
pdf(file="G_testosterone_corr_age.pdf", width=4.5, height=6)
...
ggplot(corrs_result, aes(x=reorder(Phenotype, est_diff), y=Est, color=Sex)) +
  ...
  labs(x="", y="R - Correlation between Testosterone PGS and \nEffect of Polygenic Score on Phenotype") +
