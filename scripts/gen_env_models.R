...
source("config.R")
rm(list= ls()[!(ls() %in% c('R_LIB','PHENO_DIR','LDSC_DIR','GWAS_DIR'))])
...
ldsc_df <- ldsc_df[c(1:5)] ; ldsc_df <- ldsc_df[ldsc_df$Sex != 'both_sex',]
...
df <- mutate(df, on = ifelse(pheno%in%c("albumin","calcium","eosinophil_perc","IGF1",
                                     "lymphocyte_perc","protein_total","weight"),3,on)) #check if on hypothesis
head(df)
...
# PLOTS
setwd(GWAS_DIR)
pdf(file="gen_env_models.pdf", width=7, height=5)
...
g3 <- ggplot(data=df3, aes(x= geno_var_ratio, y= env_var_ratio)) +
  ...
  #geom_segment(aes(x=ci_x1, y=ci_y1, xend=ci_x2, yend=ci_y2)) +
...
g4 <- ggplot(data=df1, aes(x= geno_var_ratio, y= env_var_ratio, color=factor(on))) +
  geom_point(size=2, alpha=0.5) + 
  geom_segment(aes(x=1,xend=2.2,y=1,yend=1),linetype="dashed", color="#207335") +   # greater male variability
  geom_segment(aes(x=1,xend=1,y=-0.24,yend=1),linetype="dashed", color="#d67629") +   # estrus mediated
  geom_abline(slope = 1, intercept = 0, color=pcolor, linetype="dashed") +
  #geom_segment(aes(x=ci_x1, y=ci_y1, xend=ci_x2, yend=ci_y2)) +
  geom_segment(aes(x=ci_x2, y=ci_y1, xend=ci_x1, yend=ci_y2)) +
  ...
  scale_color_manual(values=c("black",pcolor,"darkgray")) +
...
g5 <- ggplot(data=df2, aes(x= geno_var_ratio, y= env_var_ratio)) +
  ...
  geom_hline(yintercept = 1, linetype="dashed", color="#207335") +
  geom_abline(slope = 1, intercept = 0, color=pcolor, linetype="dashed") +
  #geom_segment(aes(x=ci_x1, y=ci_y1, xend=ci_x2, yend=ci_y2)) +
  geom_segment(aes(x=ci_x2, y=ci_y1, xend=ci_x1, yend=ci_y2)) +
  scale_x_continuous(breaks=seq(3.1,3.7,0.2), limits=c(3,3.8), expand=c(0,0)) +
  ...
  annotate("text", x=3.3, y=2.3, label = "1:1 Relationship", color= "black")
...
print("Plot, gen_env_models.pdf, in directory: ", GWAS_DIR)
