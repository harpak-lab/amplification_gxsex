...
### REGRESSION OF GENETIC EFFECT ON TESTOSTERONE PGS - OVERALL ###
source("config.R")
...
get_corr <- function(data) {
  # correlation between Beta and testosterone pgs
  corrs_row <- NULL
...
for (pheno in pheno_list$Code) {
  ...
  # sex-specific testosterone pgs scores
  ...
  # merge testosterone and phenotype dataframes
...
setwd(GWAS_DIR)
write.table(corrs_result, file="pgs_testosteronepgs_corr.txt", sep="\t", row.names=FALSE)
...
pdf(file="G_testosteronepgs_corr.pdf", width=4.5, height=6)
